...
def _in_window(row: dict, now_ms: float) -> bool:
    stamp = _timestamp_ms(row)
    return stamp > 0 and now_ms - stamp <= WINDOW_DAYS * 86_400_000


def gather_evidence(home: str | Path, agent_id: str, now_ms: float) -> dict[str, int]:
    root = _memory_dir(home)
    skills = [row for row in _rows(root / "skills.json")
              if row.get("status") == "learned" and _in_window(row, now_ms)]
    shares_sent = [row for row in _rows(root / "skill-shares.json")
                   if row.get("senderId") == agent_id and _in_window(row, now_ms)]
    conversations = [row for row in _rows(root / "conversations.jsonl")
                     if _in_window(row, now_ms)]
    playful = [row for row in _rows(root / "experiences.jsonl")
               if row.get("kind") in ("training", "train", "event", "quest", "celebration")
               and _in_window(row, now_ms)]
    builds = [row for row in _rows(root / "building.json") if _in_window(row, now_ms)]
    care = _rows(root / "care.json")
    care_resolved = [row for row in care
                     if row.get("state") in ("resolved", "closed") and _in_window(row, now_ms)]
    care_taken = [row for row in care
                  if row.get("assignedAgentId") == agent_id and _in_window(row, now_ms)]
    civic = [row for row in _rows(root / "civic.json") if _in_window(row, now_ms)]
    return {
...
def run_reflection(home: str | Path, now: datetime | None = None,
    ...
    evidence = gather_evidence(home, agent_id, now_ms)
